<?php
/**
 * LPG Module - Process Edit Record
...
try {
    ...
    $document = $stmt->fetch();
    $doc_id = $document ? $document['id'] : 0;
    
    // Check if records exist for this date
    $stmt = $db->prepare("SELECT COUNT(*) as count FROM lpg_daily_records WHERE record_date = ?");
    $stmt->execute([$record_date]);
    ...
    foreach ($numbers as $item_id => $value) {
        if ($value !== '' && $value !== null) {
            ...
            $stmt = $db->prepare("
                ...
                (doc_id, item_id, record_date, number_value, remarks, recorded_by, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
    ...
    foreach ($enums as $item_id => $value) {
        if (!empty($value)) {
            ...
            $stmt = $db->prepare("
                INSERT INTO lpg_daily_records 
                (doc_id, item_id, record_date, enum_value, remarks, recorded_by, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
    ...
    // Log activity
    logActivity($_SESSION['user_id'], 'edit_lpg_records', 
               "แก้ไขข้อมูล LPG วันที่: $record_date, จำนวน: $successCount รายการ");
    ...
    if ($isAjax) {
        ...
        header('Location: view.php?date=' . $record_date);
    }
    ...
    // Log error
    error_log("Error in LPG process_edit.php: " . $e->getMessage());
    ...
    if ($isAjax) {
        ...
        header('Location: edit.php?date=' . $record_date);
    }
}
?>